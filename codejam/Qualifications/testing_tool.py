...
def RunCases(cases, max_q):
  ...
  for i, case in enumerate(cases, 1):
    try:
      ...
      Output(CORRECT)
      tot_q += q
      # print(QUERIES_USED(tot_q,max_q), file=sys.stderr)
    except Error as err:
      Output(WRONG)
...
def main():
  ...
  try:
    ...
    # printErr(QUERIES_USED(q, Qs[index]))
  except Error as err:
    f.close()
...
# Custom function to print to stderr because I can see that. 
def printErr(*args):
