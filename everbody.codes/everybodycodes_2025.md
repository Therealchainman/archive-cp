...
int simulate() {
    ...
    for (int r = 0; r < R; ++r) {
        for (int c = 0; c < C; ++c) {
            if (grid[r][c] == '#' && cnt[r][c] % 2 == 0) {
                grid[r][c] = '.';
            } else if (grid[r][c] == '.' && cnt[r][c] % 2 == 0) {
                grid[r][c] = '#';
            }
            if (grid[r][c] == '#') ans++;
        }
...
### Solution 1: cycle, sequence, grid

1. Find the repeating sequence which is only 7 rounds that repeat with a delta of 4095 between it and the next round that matches the pattern, so given number of rounds and the number of active tiles in that round that matches the pattern can calculate answer with arithmetic. 

```cpp
int R, C, N;
vector<vector<char>> grid, pat;

bool inBounds(int r, int c) {
    return 0 <= r && r < N && 0 <= c && c < N;
}

int simulate(int round) {
    ...
    int start = N / 2 - R / 2, end = N / 2 + R / 2;
    for (int r = start; r < end; ++r) {
        for (int c = start; c < end; ++c) {
            if (grid[r][c] != pat[r - start][c - start]) return 0;
        }
    }
    debug("After round", round, ":", res, "\n");
    return res;
}
...
void solve() {
    ...
    while (getline(cin, line)) {
        pat.emplace_back(line.begin(), line.end());
    }
    R = pat.size(), C = pat[0].size();
    debug(R, C, "\n");
    int64 MAXN = 1000000000LL;
    vector<pair<int, int>> rounds;
    for (int i = 1; rounds.size() < 7; ++i) {
        int cand = simulate(i);
        if (cand) {
            rounds.emplace_back(i, cand);
        }
    }
    debug(rounds, "\n");
    int64 ans = 0;
...
struct Data {
    char c;
    int x;
    Data(char c, int x) : c(c), x(x) {}
};

vector<Data> process(const string& s, char delimiter = ' ') {
    vector<Data> ans;
    istringstream iss(s);
    string word;
    while (getline(iss, word, delimiter)) {
        ans.emplace_back(word[0], stoi(word.substr(1)));
    }
    return ans;
}
...
int bfs(int sr, int sc, int er, int ec) {
    ...
    while (!q.empty()) {
        ...
        for (int i = 0; i < sz; ++i) {
            auto [r, c] = q.front();
            // debug(steps, vis.size(), r, c, "\n");
            q.pop();
            if (r == er && c == ec) return steps;
            for (const auto& [dr, dc] : directions) {
                int nr = r + dr, nc = c + dc;
                if (walls.find({nr, nc}) != walls.end()) continue; // wall is blocking reaching here
                if (vis.find({nr, nc}) != vis.end()) continue; // already visited
                q.emplace(nr, nc);
                vis.emplace(nr, nc);
            }
        }
...
void solve() {
    string line;
    cin >> line;
    vector<Data> data = process(line, ',');
    int r = 0, c = 0, d = 0;
    // going to let it be negative
    for (const auto &[ch, x] : data) {
        if (ch == 'R') d = (d + 1) % 4;
        else d = (d - 1 + 4) % 4;
        for (int i = 0; i < x; ++i) {
    ...
    walls.erase({r, c}); // remove the final position from walls
    debug("target r, c", r, c, "\n");
    int ans = bfs(0, 0, r, c);
    debug(ans, "\n");
}
```
...
1. What data points do I need to know to help solve this? 
1. know what is to the left and down? what is in the way?
1. move in a single direction as much as you can until you hit something or overshoot the target? 
1. imagine the horizontal bars, imagine you move up, you are going to want to be inside or outside, so just move to each of those as critical points
1. Then continue your move to the left or right? 
1. the key is that you move horizontally or vertically for long distances, until you reach some critical points I think.
...
void solve() {
    ...
    getline(cin, line);
    vector<int> A = process(line, ',');
    vector<int> wall(SIZE + 1, 0);
    for (int x : A) {
        for (int i = x; i <= SIZE; i += x) {
            wall[i]++;
        }
    }
    int64 ans = accumulate(wall.begin(), wall.end(), 0LL);
    debug(ans, "\n");
}
```
...
### Solution 1: greedy, binary search, division

```cpp
const int64 MAX_BLOCKS = 202520252025000LL;
...
vector<int> getSequence(vector<int> arr) {
    arr.insert(arr.begin(), 0);
    int N = arr.size();
    vector<int> ans;
    int p = 0;
    while (any_of(arr.begin(), arr.end(), [](int x){ return x > 0; }))  {
        for (int i = p; i < N; ++i) {
            if (arr[i] > 0) {
                ans.emplace_back(i);
                for (int j = i; j < N; j += i) {
...
bool possible(const vector<int> &arr, int64 target) {
    int64 ans = 0;
    for (int x : arr) {
        int64 used = target / x;
        ans += used;
    }
    return ans <= MAX_BLOCKS;
}
...
### Solution 1: grid, radius, euclidean distance
...
// use the squaredDistance
int64 squaredDistance(int64 r1, int64 c1, int64 r2, int64 c2) {
    return (r1 - r2) * (r1 - r2) + (c1 - c2) * (c1 - c2);
}

int64 getAnswer(int64 sr, int64 sc) {
    int radius = 0;
    int64 ans = 0;
    for (int rad = 1; rad <= R / 2; ++rad) {
        int64 cur = 0;
        for (int r = 0; r < R; ++r) {
            for (int c = 0; c < C; ++c) {
                if (!grid[r][c]) continue;
                if (squaredDistance(sr, sc, r, c) <= rad * rad) {
                    cur += grid[r][c];
                    grid[r][c] = 0;
                }
            }
...
### Solution 1: dijkstra, min heap, grid, segmented, radius, euclidean distance

1. break into segments that will force it around the center of the grid.
...
map<pair<int, int>, int> segmentOne(const map<pair<int, int>, int>& dp, int centerR, int centerC, int radius) {
    vector<vector<bool>> vis(R, vector<bool>(C, false));
    map<pair<int, int>, int> segs;
    priority_queue<tuple<int, int, int>, vector<tuple<int, int, int>>, greater<>> minheap;
    for (const auto& [pos, cost] : dp) {
        auto [r, c] = pos;
        minheap.emplace(cost, r, c);
    }
    while (!minheap.empty()) {
        auto [cost, r, c] = minheap.top(); 
        minheap.pop();
        if (cost >= 30 * (radius + 1)) continue;
        if (vis[r][c]) continue;
        if (r == R / 2 && c < C / 2) {
            segs[{r, c}] = cost;
            continue;
        ...
        for (int dr = -1; dr <= 1; ++dr) {
            for (int dc = -1; dc <= 1; ++dc) {
                ...
                int dist = squaredDistance(nr, nc, centerR, centerC);
                if (dist <= radius * radius) continue;
                minheap.emplace(cost + grid[nr][nc], nr, nc);
            }
        }
    }
    return segs;
...
map<pair<int, int>, int> segmentTwo(const map<pair<int, int>, int>& dp, int centerR, int centerC, int radius) {
    vector<vector<bool>> vis(R, vector<bool>(C, false));
    map<pair<int, int>, int> segs;
    priority_queue<tuple<int, int, int>, vector<tuple<int, int, int>>, greater<>> minheap;
    for (const auto& [pos, cost] : dp) {
...
int finalSegment(const map<pair<int, int>, int>& dp, int centerR, int centerC, int radius) {
    vector<vector<bool>> vis(R, vector<bool>(C, false));
    priority_queue<tuple<int, int, int>, vector<tuple<int, int, int>>, greater<>> minheap;
    for (const auto& [pos, cost] : dp) {
        auto [r, c] = pos;
...
int64 findPath(int centerR, int centerC, int radius) {
    ...
    dp = segmentOne(dp, centerR, centerC, radius);
    dp = segmentTwo(dp, centerR, centerC, radius);
    int finalCost = finalSegment(dp, centerR, centerC, radius);
    return finalCost;
}

int64 getAnswer(int64 sr, int64 sc) {
    for (int i = 0; i < R / 2; ++i) {
        int ans = findPath(sr, sc, i);
        if (ans != INF) {
            return (ans + 1) * i;
        }
    }
...
### Solution 1: topological sort, DAG, directed graph, binary, regex, parsing

1. Solve this with observation on input data which is that all the input data has a pattern where you basically just want to only keep the positive branches from the first 81 nodes. 
1. There are like 4 layers, and you only care about positive that goes into the 2nd layer from the first. 

```cpp
int N, M, Q, freeBranches;
vector<vector<pair<int, int64>>> adj;
map<int, int> A;

int64 topo(const vector<int>& arr, vector<int> indeg) {
    vector<int64> energy(N, 0);
    energy[0] = 1;
    queue<int> q;
    assert(arr.size() == adj[0].size());
    for (int i = 0; i < freeBranches; ++i) {
        auto [v, w] = adj[0][i];
        if (--indeg[v] == 0) {
            q.emplace(v);
        }
...
void solve() {
    ...
    while (getline(cin, line)) {
        ...
        if (regex_match(line, match, plantRegex)) {
            ...
            freeBranches++;
            debug("edge: 0->", curPlant, " w=", thickness, "\n");
        } else {
    ...
    vector<int> indeg(N, 0), outSum(N, 0);
    for (const auto& [u, v, w] : edges) {
        adj[u].emplace_back(v, w);
        indeg[v]++;
        outSum[u] += w;
    }
    vector<int> bestInput(freeBranches, 0);
    for (int i = 1; i <= freeBranches; ++i) {
        if (outSum[i] > 0) bestInput[i - 1] = 1;
    }
    int64 maxEnergy = topo(bestInput, indeg);
    int64 ans = 0;
    for (const auto& input : queries) {
        int64 cur = topo(input, indeg);
        if (!cur) continue;
        ans += maxEnergy - cur;
    }
    debug(ans, "\n");
...
### Solution 1: dynamic programming, map

```cpp
vector<vector<int>> arr;

vector<int> process(const string &line) {
    vector<int> ans;
    istringstream iss(line);
    string s;
    while (getline(iss, s, ',')) {
...
void solve() {
    string line;
    while (getline(cin, line)) {
        vector<int> row = process(line);
        arr.emplace_back(std::move(row));
    }
    debug(arr, "\n");
    map<int, int> dp, ndp;
    // dp(horizontal distance, vertical distance) -> min number of flaps
    dp[0] = 0; // height -> number of flaps
    int N = arr.back().front();
    debug(N, "\n");
    for (int i = 1, j = 0; i <= N; ++i) {
        ...
        for (auto &[h, v] : dp) {
            ...
            int nheight = h - 1;
            if (i != a) {
                ndp[nheight] = ndp.find(nheight) != ndp.end() ? min(ndp[nheight], v) : v;
            } else if (nheight >= b && nheight < b + c) {
                ndp[nheight] = ndp.find(nheight) != ndp.end() ? min(ndp[nheight], v) : v;
            }
            // flap wing
            nheight = h + 1;
            if (i != a) {
                ndp[nheight] = ndp.find(nheight) != ndp.end() ? min(ndp[nheight], v + 1) : v + 1;
            } else if (nheight >= b && nheight < b + c) {
                ndp[nheight] = ndp.find(nheight) != ndp.end() ? min(ndp[nheight], v + 1) : v + 1;
            }
        }
...
### Solution 1: dynamic programming, set, math, algebra
...
void solve() {
    string line;
    int N = 0;
    map<int, set<pair<int, int>>> walls;
    while (getline(cin, line)) {
        vector<int> row = process(line);
        walls[row[0]].emplace(row[1], row[2]);
        N = max(N, row[0]);
    }
    map<int, int> dp, ndp;
    // dp(horizontal distance, vertical distance) -> min number of flaps
    ...
    for (auto it = walls.begin(); it != walls.end(); ++it) {
        int idx = it -> first;
        debug("idx: ", idx, dp.size(), "\n");
        set<pair<int, int>> data = it -> second;
        int delta = idx - pi;
        ndp.clear();
        for (auto &[h, v] : dp) {
            for (auto &[b, c] : data) {
               for (int flaps = max(0, (b - h + delta) / 2); 2 * flaps < b + c - h + delta; ++flaps) {
                    int nonflaps = delta - flaps;
                    if (nonflaps < 0) continue;
                    if (flaps + nonflaps != delta) continue;
                    int nh = h + flaps - nonflaps;
                    if (nh < b || nh >= b + c) continue;
                    ndp[nh] = ndp.find(nh) != ndp.end() ? min(ndp[nh], v + flaps) : v + flaps;
               }
            }
        }
        pi = idx;
...
### Solution 1: bfs, undirected graph, grid

```cpp
unordered_set<char> tree_chars = {'T', 'E', 'S'};

int mapToId(int r, int c, int C) {
    return r * C + c;
}
...
void solve() {
    ...
    vector<vector<int>> adj(R * C, vector<int>());
    for (int i = 0; i < R; ++i) {
        // debug(grid[i], "\n");
        int j = 0;
        while (j < C && grid[i][j] == '.') {
        ...
        for (int k = j; k < C; ++k) {
            if (tree_chars.find(grid[i][k]) == tree_chars.end()) continue;
            if (grid[i][k] == 'S') {
                source = mapToId(i, k, C);
            } else if (grid[i][k] == 'E') {
                target = mapToId(i, k, C);
            }
            if (k > 0 && tree_chars.find(grid[i][k - 1]) != tree_chars.end()) {
                adj[mapToId(i, k, C)].emplace_back(mapToId(i, k - 1, C));
                adj[mapToId(i, k - 1, C)].emplace_back(mapToId(i, k, C));
            }
            if (i > 0 && (j - k) % 2 == 0 && tree_chars.find(grid[i - 1][k]) != tree_chars.end()) {
                adj[mapToId(i, k, C)].emplace_back(mapToId(i - 1, k, C));
                adj[mapToId(i - 1, k, C)].emplace_back(mapToId(i, k, C));
            }
        }
    }
    int ans = 0;
...
### Solution 1: rotation with math and center, bfs, undirected graph, grid

```cpp
unordered_set<char> tree_chars = {'T', 'E', 'S'};

vector<vector<char>> rotate120(const vector<vector<char>> &g) {
    int R = g.size();
    int C = g[0].size();
    vector<vector<char>> res(R, vector<char>(C, '.'));

    // R rows, C = 2 * R - 1 columns, centered, bottom vertex at (R - 1, C / 2).
    for (int r = 0; r < R; ++r) {
        for (int c = 0; c < C; ++c) {
            char ch = g[r][c];
            if (ch == '.') continue;
            ...
            int nr = (c - r) / 2;
            int nc = (2 * (C - 1) - 3 * r - c) / 2;

            // should always be in bounds for valid triangle cells
...
void solve() {
    ...
    pair<int, int> source;
    debug(R, C, "\n");
    vector<vector<bool>> upwards(R, vector<bool>(C, false)), downwards(R, vector<bool>(C, false));
    for (int i = 0; i < R; ++i) {
        int j = 0;
        while (j < C && grid[i][j] == '.') {
            ++j;
        }
        for (int k = j; k < C && grid[i][k] != '.'; k += 2) {
            upwards[i][k] = true;
        }
        for (int k = j + 1; k < C && grid[i][k] != '.'; k += 2) {
            downwards[i][k] = true;
        }
        for (int c = 0; c < C; ++c) {
            if (grid[i][c] == 'S') {
    ...
    vector<vector<vector<char>>> grids = {grid};
    // rotate the grid 120 degrees two times
    for (int i = 0; i < 2; ++i) {
        vector<vector<char>> rotated = rotate120(grids.back());
        grids.emplace_back(std::move(rotated));
    }
    int ans = 0;
    queue<pair<int, int>> q;
    q.emplace(source);
    vector<vector<vector<bool>>> visited(R, vector<vector<bool>>(C, vector<bool>(3, false)));
    visited[source.first][source.second][0] = true;
    while (!q.empty()) {
        int sz = q.size();
        for (int i = 0; i < sz; ++i) {
            auto [r, c] = q.front();
            q.pop();
            if (grids[ans % 3][r][c] == 'E') {
                debug(ans, "\n");
                return;
            }
            // upwards
            if (upwards[r][c]) {
                int nr = r - 1;
                int nc = c;
                if (0 <= nr && nr < R && 0 <= nc && nc < C && tree_chars.count(grids[(ans + 1) % 3][nr][nc]) && !visited[nr][nc][(ans + 1) % 3]) {
                    visited[nr][nc][(ans + 1) % 3] = true;
                    q.emplace(nr, nc);
                }
            }
            // downwards
            ...
            // leftward
            if (c > 0 && tree_chars.count(grids[(ans + 1) % 3][r][c - 1]) && !visited[r][c - 1][(ans + 1) % 3]) {
                visited[r][c - 1][(ans + 1) % 3] = true;
                q.emplace(r, c - 1);
            }
            // rightward
            if (c + 1 < C && tree_chars.count(grids[(ans + 1) % 3][r][c + 1]) && !visited[r][c + 1][(ans + 1) % 3]) {
                visited[r][c + 1][(ans + 1) % 3] = true;
                q.emplace(r, c + 1);
            }
            // stay
            if (tree_chars.count(grids[(ans + 1) % 3][r][c]) && !visited[r][c][(ans + 1) % 3]) {
                visited[r][c][(ans + 1) % 3] = true;
                q.emplace(r, c);
            }
        }
        ans++;
