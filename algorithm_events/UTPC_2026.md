...
void solve() {
    ...
    vector<int> ans;
    for (int i = 0; i < M; ++i) {
        if ((i > 0 && A[i] == A[i - 1]) || (i + 1 < M && A[i] == A[i + 1])) {
            continue;
        }
...
void solve() {
    ...
    int cnt = 0, val = 0, i = 0;
    int64 idx = 0;
    for (const auto &[pt, d] : events) {
        int64 nidx = idx + 1LL * cnt * (pt - val);
        while (i < Q && queries[i].first < nidx) {
            auto &[x, id] = queries[i];
            ans[id] = val + (x - idx) / cnt;
            i++;
        }
        cnt += d;
        idx = nidx;
