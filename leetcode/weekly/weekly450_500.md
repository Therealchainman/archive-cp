...
1. find all palindromes of same length, can count by taking the minPrefix and current prefix
1. Determine if n itself is a palindrome
1. Count by length, not by value.
Group binary palindromes by bit-length and count how many exist for each length using simple combinatorics on the left half, avoiding any scan over k.
1. Handle the full-length boundary with prefixes.
For palindromes whose length equals bitWidth(n), compare the valid left-half prefixes up to the left half of n to count how many are strictly smaller.
1. Check the mirrored candidate once.
Construct the palindrome obtained by mirroring n’s high bits to the low side and include it if it is not greater than n.

```cpp
using int64 = long long;
class Solution {
public:
    int countBinaryPalindromes(int64 n) {
        if (!n) return 1;
        int msb = 63 - __builtin_clzll(n);
        int bitWidth = msb + 1;
        if (bitWidth == 1) return 2;
        int ans = 2;
        for (int i = 1; i <= msb / 2; ++i) {
            ...
            // length 2 * i + 1
            if (2 * i + 1 < bitWidth) {
                ans += (1LL << i); // equals 2 * (1LL << (i - 1))
            }
        }
        if (bitWidth % 2 == 0) {
            int m = bitWidth / 2;
            int64 minPrefix = 1LL << (m - 1); // m-bit, leading 1
            int64 prefix    = n >> m;         // top m bits
            if (prefix > minPrefix) ans += static_cast<int>(prefix - minPrefix);
        } else {
            int m = bitWidth / 2;
            int64 minPrefix = 1LL << m;       // (m+1)-bit, leading 1
            int64 prefix    = n >> m;         // top m+1 bits
            if (prefix > minPrefix) ans += static_cast<int>(prefix - minPrefix);
        }
        int64 cand = n;
        for (int i = 0; i < bitWidth / 2; ++i) {
            int j = msb - i;
            int b_high = (n >> j) & 1;              // read from high side
            if (b_high) cand |=  (1LL << i);        // write to low side
            else         cand &= ~(1LL << i);
        }
        if (cand <= n) ++ans;
        return ans;
...
### Solution 1: two pointers, hash set, brute force
...
### Solution 1: prefix sums, frequency counting, greedy

```cpp
class Solution {
private:
    // 1-index
    vector<vector<int>> getPrefix(const string& s) {
        int N = s.size();
...
public:
    string lexGreaterPermutation(string s, string target) {
        int N = s.size();
        vector<vector<int>> ps = getPrefix(s), pt = getPrefix(target);
        for (int i = N - 1; i >= 0; --i) {
            vector<int> f1 = ps[N], f2 = pt[i];
            vector<int> f = sub(f1, f2); // subtract f2 from f1
            if (any_of(f.begin(), f.end(), [](int x) { return x < 0; })) continue;
            int c = target[i] - 'a';
            if (all_of(f.begin() + c + 1, f.end(), [](int x) { return x == 0; })) continue;
...
### Solution 1: segment tree, prefix sum, rightmost prefix sum equal to target

1. label the even and odd with -1 and 1 so you can get sum for the farthest you can go to the right and have equal even and odd counts
1. Also track nxt occurrence of each number to update segment tree when moving left pointer
...
public:
    int longestBalanced(vector<int>& nums) {
        ...
        for (int i = 0; i < N; ++i) {
            if (seen.find(nums[i]) != seen.end()) continue; // already seen
            int s = sign(nums[i]);
            seen.insert(nums[i]);
            seg.update(i, s);
        }
        for (int l = 0; l < N; ++l) {
            // come on
            int pos = seg.rightmostEqualPrefix(0);
            if (pos != -1) {
                ans = max(ans, pos - l + 1);
            }
            seg.update(l, 0);
            if (nxt[l] != -1) {
                seg.update(nxt[l], sign(nums[l]));
            }
        }
        return ans;
    }
...
bool Solution::precomputed = false;
vector<int> Solution::last;
```

# Leetcode Weekly Contest 473

## 3727. Maximum Alternating Sum of Squares

### Solution 1: deque, sorting, greedy

```cpp
using int64 = long long;
class Solution {
public:
    int64 maxAlternatingSum(vector<int>& nums) {
        int N = nums.size();
...
### Solution 1: map, prefix sums, frequency counting

```cpp
using int64 = long long;
class Solution {
public:
    int64 countStableSubarrays(vector<int>& capacity) {
        int N = capacity.size();
        unordered_map<int, unordered_map<int, int64>> freq;
        int64 psum = 0, ans = 0;
        for (int i = 0; i < N; ++i) {
            int x = capacity[i];
            ans += freq[x][psum - x];
...
### Solution 1: prefix sums, frequency counting, queue
...
class Solution {
public:
    int64 numGoodSubarrays(vector<int>& nums, int k) {
        int N = nums.size();
        unordered_map<int, int64> freq;
        freq[0]++;
        int64 psum = 0, csum = 0, ans = 0;
        queue<int64> blockSums;
        for (int i = 0; i < N; ++i) {
            if (i > 0 && nums[i] != nums[i- 1]) {
                // delay adding prefix sums
                while (!blockSums.empty()) {
                    int64 x = blockSums.front();
                    blockSums.pop();
                    psum = (psum + x) % k;
                    freq[psum]++;
                }
...
### Solution 1: transform, sorting, multiplication of negative integers
...
public:
    int64 maxProduct(vector<int>& nums) {
        transform(nums.begin(), nums.end(), nums.begin(), [](int x) { return abs(x); });
        sort(nums.rbegin(), nums.rend());
...
### Solution 1: greedy, lcm, binary search
...
class Solution {
private:
    bool possible(int64 target, const vector<int>& d, const vector<int> r) {
        if (d[0] > target - target / r[0]) return false;
        if (d[1] > target - target / r[1]) return false;
        return d[0] + d[1] <= target - target / lcm(r[0], r[1]);
    }
public:
    int64 minimumTime(vector<int>& d, vector<int>& r) {
        int64 lo = 0, hi = INF;
        while (lo < hi) {
...
### Solution 1: palindrome, string manipulation, frequency, map

```cpp
class Solution {
private:
    map<char, int> freq, totalFreq;
    string form(const string &s) {
        map<char, int> f;
        string ans = string(s.begin(), s.end());
        for (char ch = 'a'; ch <= 'z'; ++ch) {
            while (f[ch]++ < freq[ch]) ans += ch;
        }
        return ans;
    }
    bool check(const string& s) {
        ...
        for (const char ch : s) {
            f[ch]++;
            if (f[ch] > totalFreq[ch]) return false;
        }
        return true;
    }
public:
    string lexPalindromicPermutation(string s, string target) {
        ...
        string ans;
        if (!(((N & 1) && middle.size() == 1) || (N % 2 == 0 && middle.empty()))) {
            return ans;
        }
        for (char ch = 'a'; ch <= 'z'; ++ch) {
            freq[ch] /= 2;
        ...
        for (int i = 0; i < N / 2; ++i) {
            for (char ch = target[i]; ch <= 'z'; ++ch) {
                ...
                freq[ch]--;
                cur += ch;
                string pref = form(cur);
                string suf = string(pref.rbegin(), pref.rend());
                string cand = pref + middle + suf;
                if (check(cand)) {
                    if (cand > target && ans.empty()) ans = cand;
                    else if (cand > target && cand < ans) ans = cand;
                }
                freq[ch]++;
                cur.pop_back();
            }
            // pick equal
            if (!freq[target[i]]) break;
            freq[target[i]]--;
            cur += target[i];
        }
        return ans;
...
k - i + j - i + k - j
2 * (k - i)
1. track the most recent index for the last two instances of each number iterating over nums.
...
1. dp(r, c, k)
1. O(NMK)
1. maximize cost, equals the score
1. other than standard dp does this need to be optimized? ©leetcode
...
1. dp(i, k) = max_{j <= i} (dp(j, k - 1) + max_over_range(j, i) - min_over_range(j, i))
...
## Minimum String Length After Balanced Removals©leetcode

### Solution 1: counting characters, difference in characters

```cpp
class Solution {
public:
    int minLengthAfterRemovals(string s) {
        int cnt1 = count(s.begin(), s.end(), 'a'), cnt2 = count(s.begin(), s.end(), 'b');
...
### Solution 1: digit dp, memoized recursion
...
private:
    ...
    int N;
    int64 dp[16][2][2];
    int64 dfs(int idx, bool tight, bool zero) {
        if (idx == N) return !zero;
        if (dp[idx][tight][zero] != -1) return dp[idx][tight][zero];
        int64 ans = 0;
        int cur = S[idx] - '0';
        for (int dig = 0; dig < 10; ++dig) {
            if (dig == 0 && !zero) continue;
            if (tight && dig > cur) break;
            ans += dfs(idx + 1, tight && cur == dig, zero && dig == 0);
        }
        return dp[idx][tight][zero] = ans;
    }
public:
    int64 countDistinct(int64 n) {
        S = to_string(n);
        N = S.size();
        fill(&dp[0][0][0], &dp[0][0][0] + 16 * 2 * 2, -1);
        return dfs(0, true, true);
    }
};
```

### Solution 2: combinatorial counting
...
public:
    int64 countDistinct(int64 n) {
        ...
        if (all_of(S.begin(), S.end(), [](const char &x) {
            return x != '0';
        })) ans++;
        return ans;
    }
...
## Count Stable Subarrays©leetcode

### Solution 1: prefix sum, tracking last decreasing index, counting

1. prefix sum for range queries.
1. and need an adjustments
1. and a point to the leftmost index that breaks the weakly increasing array.
...
class Solution {
public:
    vector<int64> countStableSubarrays(vector<int>& nums, vector<vector<int>>& queries) {
        int N = nums.size(), M = queries.size();
        vector<int64> values(N, 0);
        int64 cur = 0;
        for (int i = 0; i < N; ++i) {
            if (i > 0 && nums[i] < nums[i - 1]) cur = 0;
            values[i] = ++cur;
        }
        vector<int> last(N, N);
        for (int i = N - 2; i >= 0; --i) {
            last[i] = nums[i] > nums[i + 1] ? i + 1 : last[i + 1];
        }
        vector<int64> psum(N, 0);
        for (int i = 0; i < N; ++i) {
            psum[i] = values[i];
        ...
        for (int i = 0; i < M; ++i) {
            ...
            int64 cand = psum[r] - (l > 0 ? psum[l - 1] : 0);
            if (l > 0) {
                int64 v = values[l - 1];
                int64 len = min(last[l - 1], r + 1) - l;
                cand -= v * len;
            }
            ans[i] = cand;
        }
...
1. It is easy to know the total number of subsequences it is 2^n - 1
1. What if you just calculate the number of subsequences that contain are equal to the or of all the elements. 
1. 
...
# Leetcode Weekly Contest 479

## Largest Prime from Consecutive Prime Sum

### Solution 1: prime sieve, greedy

1. largest consecutive primes
1. need to generate a list of all primse and if they are prime up to 1e5.
1. prime sieve
...
private:
    ...
    void sieve() {
        ...
        for (int p = 2; 1LL * p * p <= MAXN; ++p) {
            ...
            for (int64 i = 1LL * p * p; i < MAXN; i += p) {
                primes[i] = false;
            }
        }
        for (int p = 2; p < MAXN; ++p) {
            if (primes[p]) pool.emplace_back(p);
        }
        precomputed = true;
    }
public:
    int largestPrime(int n) {
        ...
        for (int x : pool) {
            cur += x;
            if (cur > n) break;
            if (primes[cur]) ans = cur;
        }
        return ans;
    }
};
bool Solution::primes[MAXN];
vector<int> Solution::pool;
bool Solution::precomputed = false;
```

## Total Score of Dungeon Runs

### Solution 1: reversing, prefix sum, binary search

1. reverse damage and requirements. 
1. prefix sum is monotonically increasing
1. binary search for each query, NlogN
...
private:
    ...
    bool possible(int hp, int req, int64 pref) {
        return hp >= req + pref;
    }
public:
    int64 totalScore(int hp, vector<int>& A, vector<int>& B) {
        int N = A.size();
        ...
        for (int i = 0; i < N; ++i) {
            ...
            while (lo < hi) {
                int mid = lo + (hi - lo) / 2;
                if (possible(hp, B[i], rangeSum(psum, i, mid))) {
                    lo = mid + 1;
                } else {
                    hi = mid;
                }
...
### Solution 1: reroot tree dp, tree, dfs

1. subgraph, good minus bad
1. for each node find maximum score among all connected subgraphs that contain node i.
1. reroot dp on tree algorithm

```cpp
class Solution {
private:
    vector<vector<int>> adj;
    vector<int> S, good, ans;
    ...
    void dfs2(int u, int p = -1, int par = 0) {
        ans[u] = S[u] + max(0, par);
        for (int v : adj[u]) {
            if (v == p) continue;
            int newPar = max(0, par) + S[u] - max(0, S[v]);
            dfs2(v, u, newPar);
        }
    }
public:
...
### Solution 1: set, check sum has been seen
...
### Solution 1: counting pairs, string transformation, sorting, grouping
...
public:
    int64 countPairs(vector<string>& words) {
        int N = words.size();
        vector<vector<int>> arr;
        for (const string &word : words) {
            vector<int> A;
            int delta = 'z' - word[0] + 1;
            for (char ch : word) {
                int d = ch - 'a';
                d += delta;
                d %= 26;
...
### Solution 1: greedy, bit manipulation, most significant bit, sorting

```cpp
using int64 = long long;
class Solution {
private:
    int calc(int64 x, int64 target) {
        int64 missing = ~x & target;
        if (!missing) return 0;
        int msb = 63 - __builtin_clzll(missing);
        int64 lowerMask = (1LL << (msb + 1)) - 1;
        int64 cand = (x & ~lowerMask) | (lowerMask & target);
        return cand - x;
    }
    bool possible(const vector<int> &nums, int k, int m, int mask) {
        int N = nums.size();
        vector<int> costs(N, 0);
        for (int i = 0; i < N; ++i) {
            costs[i] = calc(nums[i], mask);
        }
...
public:
    int maximumAND(vector<int>& nums, int k, int m) {
        ...
        for (int i = 30; i >= 0; --i) {
            int cand = ans | (1 << i);
            if (possible(nums, k, m, cand)) ans = cand;
        }
        return ans;
    }
};
...
# Leetcode Weekly Contest 485

## 3814. Maximum Capacity Within Budget

### Solution 1: sorting, binary search

sorting the events by increasing costs
storing the prefix max over capacity for those sorted events.
Consider taking each ith event with the (cost_i, capacity_i) and then binary search to find the prefix you can consider taking where j < i, and max(A[0...j])
And this will consider the best option for taking ith and jth element, but also can build in if you take just i by itself.
...
public:
    int maxCapacity(vector<int>& costs, vector<int>& capacity, int budget) {
        ...
        int N = costs.size();
        for (int i = 0; i < N; ++i) {
            events.emplace_back(costs[i], capacity[i]);
        }
        sort(events.begin(), events.end());
        sort(costs.begin(), costs.end());
        vector<int> pmax(N + 1, 0);
        for (int i = 0; i < N; ++i) {
            pmax[i + 1] = max(pmax[i], events[i].second);
        }
        int ans = 0;
        for (int i = 0; i < N; ++i) {
            if (events[i].first >= budget) continue;
            int j = lower_bound(costs.begin(), costs.end(), budget - events[i].first) - costs.begin();
            j = min(j, i);
            ans = max(ans, events[i].second + pmax[j]);
        }
        return ans;
    }
};
...
Just using map and sets.

Use a map of sets of itemId -> (bidAmount, userId) so that I can always get the highest bidder for a specific itemId, just taking the first element in the corresponding set.

```cpp
class AuctionSystem {
private:
    unordered_map<int, set<pair<int, int>, greater<pair<int, int>>>> itemBids;
    map<pair<int, int>, int> userBids;
public:
    AuctionSystem() {
        itemBids.clear();
        userBids.clear();
    ...
    void addBid(int userId, int itemId, int bidAmount) {
        pair<int, int> userKey = {userId, itemId};
        if (userBids.find(userKey) != userBids.end()) {
            int bid = userBids[userKey];
            itemBids[itemId].erase({bid, userId});
        }
        itemBids[itemId].emplace(bidAmount, userId);
        userBids[userKey] = bidAmount;
    }
    ...
    void updateBid(int userId, int itemId, int newAmount) {
        ...
        int bid = userBids[userKey];
        itemBids[itemId].erase({bid, userId});
        itemBids[itemId].emplace(newAmount, userId);
        userBids[userKey] = newAmount;
    }
    ...
    int getHighestBidder(int itemId) {
        ...
        auto [bid, userId] = *itemBids[itemId].begin();
        return userId;
    }
};
```
...
### Solution 1: greedy, strings, sliding window

We want the lexicographically smallest subsequence that contains every distinct character of s at least once
maintain a sliding window, and greedily pick the smallest next character within that window. 
The window is where you must next take a character.
The left endpoint of the window is just based on the last character you took, so basically you can only pick a new character to the right of the last character picked.
The right endpoint of the window is defined by the fact that for every character we still must pick, we can’t go past its last occurrence, otherwise we’d lose it forever.
...
public:
    string lexSmallestAfterDeletion(string s) {
        int N = s.size();
        string ans;
        vector<bool> required(26, false);
        vector<vector<int>> pos(26, vector<int>());
        vector<int> ptr(26, 0), last(26, 0);
        for (int i = 0; i < N; ++i) {
            int x = s[i] - 'a';
            required[x] = true;
        ...
        int start = 0;
        while (any_of(required.begin(), required.end(), [](const bool x) { return x; })) {
            int limit = N - 1;
            for (int i = 0; i < 26; ++i) {
                if (required[i]) {
                    limit = min(limit, last[i]);
                }
            }
            // take smallest char in interval [start, limit]
            for (int i = 0; i < 26; ++i) {
                while (ptr[i] < pos[i].size() && pos[i][ptr[i]] < start) {
                    ptr[i]++;
                }
            }
            for (int i = 0; i < 26; ++i) {
                if (ptr[i] >= pos[i].size()) continue;
                int p = pos[i][ptr[i]]; // p >= start
                if (p <= limit) {
                    start = p + 1;
                    required[i] = false;
                    ans += 'a' + i;
...
# Leetcode Weekly Contest 486

## 3819. Rotate Non Negative Elements

### Solution 1: modulo, indexing

Finding the new index for each non-negative integer under modulo so that it can wrap around. 
...
### Solution 1: dp on tree, rerooting tree, pythagorean theorem, undirected graph, tree

The main trick is that when you are moving down the tree, if you are moving into a subtree that contains that element you must decrease the distance to it, else you increase the distance, because you are moving farther away or closer to the element, depending if it is in the subtree you are moving to or it is in a node not in the subtree. 

```cpp
class Solution {
private:
    vector<vector<int>> adj;
    vector<bool> hx, hy, hz;
    int dx, dy, dz, x, y, z, ans;
    ...
    bool isPythagorean(int aa, int bb, int cc) {
        vector<int> A = {aa, bb, cc};
        sort(A.begin(), A.end());
        int a = A[0], b = A[1], c = A[2];
        return 1LL * a * a + 1LL * b * b == 1LL * c * c;
    }
    void dfs1(int u, int p, int dx, int dy, int dz) {
        if (isPythagorean(dx, dy, dz)) ans++;
        for (int v : adj[u]) {
            if (v == p) continue;
            int ndx = hx[v] ? dx - 1 : dx + 1;
            int ndy = hy[v] ? dy - 1 : dy + 1;
            int ndz = hz[v] ? dz - 1 : dz + 1;
            dfs1(v, u, ndx, ndy, ndz);
        }
    }
public:
    int specialNodes(int n, vector<vector<int>>& edges, int xx, int yy, int zz) {
        adj.assign(n, vector<int>());
        x = xx, y = yy, z = zz;
        for (const vector<int>& edge : edges) {
            int u = edge[0], v = edge[1];
...
### Solution 2: depth in rooted tree, dfs

dfs works well because you just want to find the distance from three nodes, so take one node and treat that as the root, and as you dfs track the depth, the depth is the distance to all other nodes from the source node. 

This is an example when dfs works well to perform shortest path between a source node and every other node, because it is a tree. However, it is not shortest path in the general sense, it's distance along the unique path which automatically equals shortest because not alternative path exists.

So if you root the tree at s and run DFS while carrying depth, the first (and only) time you reach u, the depth you have equals the length of that unique path. That’s why d[u] = depth is the true shortest distance in a tree.

```cpp
class Solution {
private:
    vector<vector<int>> adj;
    void dfs(int u, vector<int> &d, int p = -1, int depth = 0) {
        d[u] = depth;
        for (int v : adj[u]) {
            if (v == p) continue;
            dfs(v, d, u, depth + 1);
...
public:
    int specialNodes(int n, vector<vector<int>>& edges, int x, int y, int z) {
        ...
        dfs(x, dx), dfs(y, dy), dfs(z, dz);
        int ans = 0;
        for (int i = 0; i < n; ++i) {
            if (isPythagorean(dx[i], dy[i], dz[i])) {
                ans++;
            }
        }
        return ans;
...
### Solution 1: greedy, lexicographical, unranking algorithm

A natural prefix is: “the bits you’ve already decided from MSB down to some position”.

Numeric order of bitstrings: at each bit, try 0 then 1 (because 0 < 1).
Subtree sizes vary wildly, but the order is fixed.

Think about a decision tree, where at each step there are two children nodes, 0 or 1, where you want to try 0 first and then 1, to satisfy lexicographical ordering. 

“the first node that you can go down because the number of combinations that can be generated is less than or equal to n”

Anytime you skip a path such as a 0, you have to subtract it , because you lose all of those configurations. 

When you skip you have to subtract that number of ways that would have been generated, 

REmember if cnt < n, you have to subtract from n, cause you have to skip you can't go down that path, all those configurations with 0 at i, and all of that is skipped now. 

Because n is a position in an ordered list, and the 0-child subtree is a whole contiguous prefix of that list at that decision point.
...
public:
    int64 nthSmallest(int64 n, int k) {
        ...
        for (int i = 49; i >= 0 && k > 0; --i) {
            ...
            // go down 0th node.
            if (ways >= n) continue;
            // go down 1th node path, and skip 0th node.
            ans |= (1LL << i);
            n -= ways;
            k--;
        }
...
# Leetcode Weekly Contest 487

## 3828. Final Element After Subarray Deletions

### Solution 1: maximum

I figured this out peronsally by considering how Alice could even get any other element but the endpoints.  But the thing is that Bob will always be able to block that. 
...
Using a queue and tracking the active riders in a set.
...
public:
    ...
    vector<int> matchDriverWithRider() {
        while (!rq.empty() && active.find(rq.front()) == active.end()) {
            rq.pop();
        }
        if (rq.empty() || dq.empty()) return {-1, -1};
        int u = dq.front(), v = rq.front();
        dq.pop();
        rq.pop();
        return {u, v};
    }
...
### Solution 1: dynamic programming, forward and backwards dp values.

I find the easiest way to solve is by tracking the longest subsequence at each index, thus that element was larger than previous or smaller. And do same from reverse direction.

It is easy to simulate the removal of one element, by combining the opposites from forward and backward.  So the longest length with last element being less than previous, and longest length with last element being larger than next.
...
public:
    int longestAlternating(vector<int>& nums) {
        int N = nums.size();
        vector<int> I(N, 1), D(N, 1), rI(N, 1), rD(N, 1);
        for (int i = 1; i < N; ++i) {
            if (nums[i] > nums[i - 1]) {
                I[i] = D[i - 1] + 1;
        ...
        int ans = max(*max_element(I.begin(), I.end()), *max_element(D.begin(), D.end()));
        for (int i = 1; i + 1 < N; ++i) {
            ans = max({ans, I[i], D[i]});
            if (nums[i + 1] > nums[i - 1]) {
                // i - 1 needs to be smaller than i - 2
                // i + 1 needs to be greater than i + 2.
                ans = max(ans, D[i - 1] + rI[i + 1]);
            } else if (nums[i + 1] < nums[i - 1]) {
                // i - 1 needs to be greater than i - 2.
                // i + 1 needs to be smaller than i + 2.
                ans = max(ans, I[i - 1] + rD[i + 1]);
            }
        }
        return ans;
    }
...
# Leetcode Weekly Contest 488

## Merge Adjacent Equal Elements

### Solution 1: stack simulation, collapsing on stack

stack simulation I think is what this is called. You track a current running sum for your block, and maintain a stack ans of finished blocks.  You have to collapse repeatedly whenever the top of the stack is equal to the current block. You determine a block is finalized and pushed back on stack when cur != nums[i + 1].

```cpp
using int64 = long long;
class Solution {
public:
    vector<int64> mergeAdjacent(vector<int>& nums) {
        int N = nums.size();
        int64 cur = 0;
        vector<int64> ans;
        for (int i = 0; i < N; ++i) {
...
### Solution 1: max and min monotonic deque, sliding window

sliding window with max/min monotonic deques, if you extend the right endpoint it is always guaranteed that either the max increases, min decreases, and length increases, which means the cost is monotonic it can only get larger, so when it gets too large, you just need to move the left endpoint until it works, and then all subarrays in the [l, r] will work. 
...
private:
    ...
    int64 cost(int l, int r) {
        return 1LL * (arr[maxQueue.front()] - arr[minQueue.front()]) * (r - l + 1);
    }
public:
    int64 countSubarrays(vector<int>& nums, int64 k) {
        int N = nums.size();
        arr.assign(nums.begin(), nums.end());
        int64 ans = 0;
        for (int l = 0, r = 0; r < N; ++r) {
            while (!maxQueue.empty() && nums[r] >= nums[maxQueue.back()]) {
                maxQueue.pop_back();
            }
            while (!minQueue.empty() && nums[r] <= nums[minQueue.back()]) {
                minQueue.pop_back();
            }
            maxQueue.emplace_back(r);
            minQueue.emplace_back(r);
            while (l <= r && cost(l, r) > k) {
                if (!maxQueue.empty() && maxQueue.front() == l) {
                    maxQueue.pop_front();
                }
                if (!minQueue.empty() && minQueue.front() == l) {
                    minQueue.pop_front();
                }
                l++;
            }
...
dynamic programming, dp[i][j][k] = maximum score when you have picked exactly k pairs in the subarray nums1[0...i] and nums2[0...j], the transitions are going to be from dp[i][j][k] = max(dp[i - 1][j - 1][k], dp[i][j - 1][k], dp[i - 1][j][k], dp[i - 1][j - 1][k - 1] + nums1[i] * nums2[j])
Meaning up to this two subarray this is max possible if I've picked my exaclty k pairs in nums1[0...i] and nums2[0...j], and it can be pick the current (i, j). 
...
class Solution {
public:
    int64 maxScore(vector<int>& nums1, vector<int>& nums2, int k) {
        int N = nums1.size(), M = nums2.size();
        vector<vector<int64>> dp(N + 1, vector<int64>(M + 1, 0)), ndp(N + 1, vector<int64>(M + 1, -INF));
        while (k > 0) {
            ndp.assign(N + 1, vector<int64>(M + 1, -INF));
            for (int i = 1; i <= N; ++i) {
                for (int j = 1; j <= M; ++j) {
                    int64 cand = 1LL * nums1[i - 1] * nums2[j - 1];
                    ndp[i][j] = max({ndp[i - 1][j - 1], ndp[i][j - 1], ndp[i - 1][j]});
                    if (dp[i - 1][j - 1] != -INF) ndp[i][j] = max(ndp[i][j], dp[i - 1][j - 1] + cand);
                }
            }
            swap(dp, ndp);
            k--;
...
### Solution 1: map, frequency counting, and inverse frequency counting

Finding frequency of each element, and then the frequency of frequencies, and then checking for each element if the frequency of its frequency is 1, meaning it is the only element with that frequency, thus it is the first unique frequency.
...
### Solution 1: expanding out around center, longest palindrome with one deletion

Expand out around center until you reach first mismatch, then try both candidates of deleting the left or right character and continue to expand as far as you can. 
...
private:
    ...
    int calc(int i, int j, const string& s) {
        int N = s.size();
        while (i >= 0 && j < N && s[i] == s[j]) {
            --i, ++j;
        }
        if (i < 0 || j >= N) {
            return j - i + (i >= 0 || j < N ? 0 : -1);
        }
        // delete a character from i or j
        int c1 = expand(i - 1, j, s);
        int c2 = expand(i, j + 1, s);
        return max(c1, c2);
    }
public:
    int almostPalindromic(string s) {
        int N = s.size(), ans = 1;
        for (int i = 0; i < N; ++i) {
            ans = max(ans, calc(i, i, s));
            if (i > 0) ans = max(ans, calc(i - 1, i, s));
        }
        return ans;
    }
};
...
### Solution 1: max and min deque, sliding window, trie for maximum subarray xor, uses prefix xor
...
public:
    int maxXor(vector<int>& nums, int k) {
        ...
        for (int i = 0, j = 0; i < N; ++i) {
            ...
            while (!minWindow.empty() && nums[i] <= nums[minWindow.back()]) {
                minWindow.pop_back();
            }
            maxWindow.emplace_back(i);
            minWindow.emplace_back(i);
            while (nums[maxWindow.front()] - nums[minWindow.front()] > k) {
                trie.erase(pref[j]);
                if (maxWindow.front() == j) maxWindow.pop_front();
                if (minWindow.front() == j) minWindow.pop_front();
                j++;
            }
            ans = max(ans, trie.find(pref[i + 1]));
        }
        return ans;
    }
};
