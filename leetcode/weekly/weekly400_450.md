...
struct Node {
    ...
    constexpr Node& operator+=(const Node& rhs) noexcept {
        x += rhs.x;
        y += rhs.y;
        return *this;
    }
...
public:
    int numberOfSubmatrices(vector<vector<char>>& grid) {
        ...
        vector<vector<Node>> dp(R, vector<Node>(C));
        for (int i = 0; i < R; i++) {
            for (int j = 0; j < C; j++) {
...
### Solution 1: greedy, binary search, radius, overlapping intervals

Pin one center, then greedily cover the rest.
You can always place x = min(S) + d. That first interval covers [min(S), min(S) + 2d]. Whatever is left must be a single block to the right. Check if one more interval of length 2d covers it. Equivalently, verify last - first_uncovered <= 2d. If it fits, feasible.
...
private:
    int N;
    vector<int> nums;
    bool possible(int target, int x, int y) {
        int prv = 0, cnt = 0;
        for (int i = 0; i < N; ++i) {
            if (nums[i] == -1) {
                if (prv && min(abs(prv - x), abs(prv - y)) > target) return false; // trailing -1
                cnt++;
                continue;
            }
            if (cnt) {
                int diff = prv ? min(max(abs(nums[i] - x), abs(prv - x)), max(abs(nums[i] - y), abs(prv - y))) : min(abs(nums[i] - x), abs(nums[i] - y));
                if (diff > target && (cnt == 1 || y - x > target)) return false;
            }
            prv = nums[i];
            cnt = 0;
        }
...
public:
    int minDifference(vector<int>& A) {
        ...
        int lo = maxGap, hi = (r - l + 1) / 2;
        while (lo < hi) {
            int mid = lo + (hi - lo) / 2;
            if (possible(mid, l + mid, r - mid)) {
                hi = mid;
            } else {
                lo = mid + 1;
            }
...
### Solution 1: modulo, prefix minimum, prefix sum
...
public:
    int64 maxSubarraySum(vector<int>& nums, int k) {
        ...
        int N = nums.size();
        int64 pref = 0, ans = -INF;
        vector<int64> minPref(k, INF);
        minPref[0] = 0;
        for (int i = 1; i < N; ++i) {
            pref += nums[i];
            if (i >= k) {
                ans = max(ans, pref - minPref[i % k]);
            }
            minPref[i % k] = min(minPref[i % k], pref);
        }
        return ans;
    }
};
...
### Solution 1: multinomial coefficient, combinatorics, unranking permutations, multiset permutations

Unranking: pick the next character by counting how many permutations start with it

unranking of permutations with duplicates.

multiset permutations

In combinatorics, a **ranking** function maps each object in an ordered family to its position number.  
Example: list all permutations of `abc` in lexicographic order
1. `abc`
2. `acb`
3. `bac`
4. `bca`
...
6. `cba`
Here `rank("bca") = 4`.
**Unranking** is the inverse task. You are given a rank `k` and the ordering rule, and you reconstruct the `k`-th object without listing all previous ones.

Your algorithm is unranking because it starts with `k` and, step by step, chooses the next character by counting how many completions each choice would produce. If the block size for a choice is smaller than `k`, you subtract it from `k` and move on. When a block is at least `k`, you fix that choice and continue. In the end you have built exactly the `k`-th item in the order. For palindromic permutations the “objects” are the left halves of the palindrome and the counts come from multinomial coefficients.
...
public:
    string smallestPalindrome(string s, int k) {
        ...
        if (multinomial(freq) < k) return left;
        for (int i = 0; i < N / 2; ++i) {
            for (int j = 0; j < 26; ++j) {
                if (!freq[j]) continue;
                freq[j]--;
                int ways = multinomial(freq);
                if (ways < k) {
                    k -= ways;
                    freq[j]++;
                } else {
...
### Solution 1:  dp bitmask, bit manipulation, remainder, long division

1. Subset DP: for each subset of indices and remainder r mod k, store the smallest-by-lexicographic-order sequence of chosen numbers that yields r when concatenated. Base is empty subset with remainder 0.
2. Concatenation math: precompute pow10[i] = 10^(digits in nums[i]). Appending nums[i] updates remainder as new = (old * pow10[i] + nums[i]) mod k.
3. Result selection: iterate transitions by adding any unused index; keep only the best sequence per state. Return the sequence at the full set with remainder 0, or empty if none exists.
...
public:
    vector<int> concatenatedDivisibility(vector<int>& nums, int k) {
        ...
        vector<unordered_map<int, vector<int>>> dp(1 << N, unordered_map<int, vector<int>>());
        dp[0][0] = {};
        for (int mask = 0; mask < (1 << N); ++mask) {
            for (auto [rem, seq] : dp[mask]) {
                for (int i = 0; i < N; ++i) {
                    if ((mask >> i) & 1) continue;
                    int nmask = mask | (1 << i);
                    int nrem = (rem * pow10[i] + nums[i]) % k;
                    vector<int> nseq = seq;
                    nseq.emplace_back(nums[i]);
                    if (!dp[nmask].count(nrem) || nseq < dp[nmask][nrem]) {
                        dp[nmask][nrem] = nseq;
                    }
                }
            } 
        ...
        int endmask = (1 << N) - 1;
        return dp[endmask].count(0) ? dp[endmask][0] : vector<int>();
    }
};
```
...
public:
    vector<int> pathExistenceQueries(int n, vector<int>& nums, int maxDiff, vector<vector<int>>& queries) {
        ...
        vector<int> getIndex(n, 0); // node u -> i in the sorted array
        for (int i = 0; i < n; ++i) {
            getIndex[nodes[i].second] = i;
        }
        sort(nums.begin(), nums.end());
        for (int i = 0, j = 0; i < n; ++i) {
            succ[0][i] = -1;
            while (nums[i] - nums[j] > maxDiff) {
                succ[0][j] = i - 1; // can reach the last one
                j++;
            }
        }
        for (int i = 0; i < n; ++i) {
            if (succ[0][i] == -1) succ[0][i] = n - 1; // can reach the last
        }
        for (int i = 1; i < LOG; ++i) {
            for (int j = 0; j < n; ++j) {
...
### Solution 1: combinatorics, multinomial, binary digit dp, carry, bit manipulation

This DP is exactly “addition in base 2, bit by bit, carrying upward”, done in grouped counts c at each power 2^i. In the algorithms world this pattern is usually framed as:
- DP over binary digits with carry.
- add the number with a 1 at bit i, c_i times
...
private:
    ...
    int64 dfs(int i, int m, int k, int carry) {
        if (m == 0) {
            return k == __builtin_popcount(carry) ? 1 : 0;
        }
        if (i == N || k < 0) return 0;
        if (dp[i][m][k][carry] != -1) return dp[i][m][k][carry];
        int64 ans = 0;
        for (int c = 0; c <= m; ++c) {
            int ncarry = carry + c;
            int bit = ncarry % 2;
            int64 mult = inv_fact[c] * exponentiation(A[i], c, MOD) % MOD;
            ans = (ans + mult * dfs(i + 1, m - c, k - bit, ncarry / 2)) % MOD;
        }
        return dp[i][m][k][carry] = ans;
    }
public:
    int magicalSum(int m, int k, vector<int>& nums) {
        N = nums.size();
        fill(&dp[0][0][0][0], &dp[0][0][0][0] + 50 * 31 * 31 * 16, -1);
        A = nums;
        factorials(m, MOD);
        return fact[m] * dfs(0, m, k, 0) % MOD;
    }
};
```
