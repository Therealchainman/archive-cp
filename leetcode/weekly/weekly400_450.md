...
### Solution 1: dynamic programming, 2d dp, diagonals, grid
...
private:
    ...
    vector<vector<int>> dp1, dp2, dp3, dp0;
    bool in_bounds(int r, int c) {
        return 0 <= r && r < R && 0 <= c && c < C;
    }
    void precompute() {
        dp1.assign(R, vector<int>(C, 0));
        dp2.assign(R, vector<int>(C, 0));
        dp3.assign(R, vector<int>(C, 0));
        dp0.assign(R, vector<int>(C, 0));
        for (int nr = 0; nr < R; nr++) {
            auto [dr, dc] = DIRECTIONS[3];
            int r = nr;
            int c = 0;
            dp3[r][c] = max(dp3[r][c], 1);
            r += dr; c += dc;
            while (in_bounds(r, c)) {
                dp3[r][c] = max(dp3[r][c], 1);
                if (grid[r][c] == 0 && grid[r - dr][c - dc] == 2) {
                    dp3[r][c] = max(dp3[r][c], dp3[r - dr][c - dc] + 1);
                } else if (grid[r][c] == 2 && grid[r - dr][c - dc] == 0) {
                    dp3[r][c] = max(dp3[r][c], dp3[r - dr][c - dc] + 1);
                }
                r += dr; c += dc;
            }
        ...
        for (int nr = 0; nr < R; nr++) {
            auto [dr, dc] = DIRECTIONS[1];
            int r = nr;
            int c = C - 1;
            dp1[r][c] = max(dp1[r][c], 1);
            r += dr; c += dc;
            while (in_bounds(r, c)) {
                dp1[r][c] = max(dp1[r][c], 1);
                if (grid[r][c] == 0 && grid[r - dr][c - dc] == 2) {
                    dp1[r][c] = max(dp1[r][c], dp1[r - dr][c - dc] + 1);
                } else if (grid[r][c] == 2 && grid[r - dr][c - dc] == 0) {
                    dp1[r][c] = max(dp1[r][c], dp1[r - dr][c - dc] + 1);
                }
                r += dr; c += dc;
        ...
        for (int nr = 0; nr < R; nr++) {
            auto [dr, dc] = DIRECTIONS[0];
            int r = nr;
            int c = 0;
            dp0[r][c] = max(dp0[r][c], 1);
            r += dr; c += dc;
            while (in_bounds(r, c)) {
                dp0[r][c] = max(dp0[r][c], 1);
                if (grid[r][c] == 0 && grid[r - dr][c - dc] == 2) {
                    dp0[r][c] = max(dp0[r][c], dp0[r - dr][c - dc] + 1);
                } else if (grid[r][c] == 2 && grid[r - dr][c - dc] == 0) {
                    dp0[r][c] = max(dp0[r][c], dp0[r - dr][c - dc] + 1);
                }
                r += dr; c += dc;
        ...
        for (int nr = 0; nr < R; nr++) {
            auto [dr, dc] = DIRECTIONS[2];
            int r = nr;
            int c = C - 1;
            dp2[r][c] = max(dp2[r][c], 1);
            r += dr; c += dc;
            while (in_bounds(r, c)) {
                dp2[r][c] = max(dp2[r][c], 1);
                if (grid[r][c] == 0 && grid[r - dr][c - dc] == 2) {
                    dp2[r][c] = max(dp2[r][c], dp2[r - dr][c - dc] + 1);
                } else if (grid[r][c] == 2 && grid[r - dr][c - dc] == 0) {
                    dp2[r][c] = max(dp2[r][c], dp2[r - dr][c - dc] + 1);
                }
                r += dr; c += dc;
    ...
    int bfs(int r, int c) {
        int cur = 0, ans = 1;
        int dr, dc;
        tie(dr, dc) = DIRECTIONS[0];
        int v = 2;
        for (int nr = r + dr, nc = c + dc; in_bounds(nr, nc); nr += dr, nc += dc) {
            if (grid[nr][nc] != v) break;
            cur++;
            v = v == 2 ? 0 : 2;
            ans = max(ans, cur + dp3[nr][nc]);
        }
        tie(dr, dc) = DIRECTIONS[1];
        v = 2;
        cur = 0;
        for (int nr = r + dr, nc = c + dc; in_bounds(nr, nc); nr += dr, nc += dc) {
            if (grid[nr][nc] != v) break;
            cur++;
            v = v == 2 ? 0 : 2;
            ans = max(ans, cur + dp0[nr][nc]);

        }
        tie(dr, dc) = DIRECTIONS[2];
        v = 2;
        cur = 0;
        for (int nr = r + dr, nc = c + dc; in_bounds(nr, nc); nr += dr, nc += dc) {
            if (grid[nr][nc] != v) break;
            cur++;
            v = v == 2 ? 0 : 2;
            ans = max(ans, cur + dp1[nr][nc]);
        }
        tie(dr, dc) = DIRECTIONS[3];
        v = 2;
        cur = 0;
        for (int nr = r + dr, nc = c + dc; in_bounds(nr, nc); nr += dr, nc += dc) {
            if (grid[nr][nc] != v) break;
