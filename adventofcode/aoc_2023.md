...
pipe_mapping = {key: [(y, x) for x, y in value] for key, value in pipe_mapping.items()}
...
def solve(data):
    ...
    dist[r0][c0] = 0
    queue = deque([(r0, c0, -1, -1)])
    in_bounds = lambda r, c: 0 <= r < R and 0 <= c < C
    while queue:
        r, c, pr, pc = queue.popleft()
        pipe = data[r][c]
        for dr, dc in pipe_mapping[pipe]:
            nr, nc = r + dr, c + dc
            if not in_bounds(nr, nc): continue
            if (nr, nc) == (pr, pc): continue # parent
            if data[nr][nc] not in valid_pipes[(dr, dc)]: continue
            if dist[nr][nc] == math.inf:
                dist[nr][nc] = dist[r][c] + 1
                queue.append((nr, nc, r, c))
            elif abs(dist[nr][nc] - dist[r][c]) > 1: continue
            else: return dist[nr][nc]
