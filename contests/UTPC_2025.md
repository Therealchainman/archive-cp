...
1. find gcd of every subarray, determine which subarrays have gcd = 1. 
1. O(nk) if you just dp[n][k], yeah? but no that wouldn't work you need current gcd and when you are starting and ending current subarray? 
1. given index i, query to find the smallest index to right, j such that gcd of that subarray is 1? 
1. binary search with sparse table for gcd, cause gcd will be weakly decreasing as you expand the subarray to the right for any index i. 
